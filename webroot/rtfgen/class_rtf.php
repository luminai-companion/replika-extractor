...
class rtf {
	...
	var $filename="rtf.rtf";
	
	function __construct($config="rtf_config.php",$filename = "rtf.rtf") {
		require_once($config);
	...
	function escapeCharacter($character) {
		...
		if (ord($character) >= 0x80 and ord($character) < 0xFF){
			//echo "Bad character: $character with ".ord($character)."<br>\n";
			$escaped = "\\'".dechex(ord($character));
		}
	...
	function parseDocument() {
		...
		$doc_buffer = preg_replace("/<S>(.*?)<\/S>/mi", "\\strike \\1\\strike0 ", $doc_buffer);
