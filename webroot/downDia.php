...
$rtf = new rtf("rtf_config.php","diary.rtf");
...
if($downloadType == "rtf"){
	...
	$rtf->setTitle("Diary of $user_name and $bot_name");
	$rtf->addColour("#000000");//cf0 & cf1
	$rtf->addColour("#0000FF");//cf2
	$rtf->addColour("#FF0000");//cf3
	$rtf->addText("<p><b>Diary of $user_name and $bot_name</b></p>");
}
...
$sql = "SELECT * FROM diary WHERE user_id='".$user_id."' AND parent_id<>'0' ORDER BY parent_id, diary_id";
...
for($line=0;$row = db_fetch_assoc($q);$line++){
	...
	if($downloadType == "rtf"){
		...
		$rtf->addText("<p>[".$row['entrydate']."] ");

		$rtf->addText($textToAdd);
		$rtf->addText("</p>");
	}
	if($downloadType == "sql"){
		$output .= "INSERT INTO diary SET ";
		$delim = "";
		foreach($row AS $k => $v){
...
if($downloadType == "sql"){
	...
	header("Content-Disposition: attachment; filename=diary.sql");
	echo $output;
...
if($downloadType == "csv"){
	...
	header("Content-Disposition: attachment; filename=diary.csv");
	echo $output;
